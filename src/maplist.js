  ...
  (function($, global) {
    ...
    Parser = (function() {
      ...
      Parser.prototype.execute = function(data) {
        var sequence, _ref, _ref1;
        sequence = [];
        sequence.push((_ref = this.options.parser) != null ? _ref : new Parser.DefaultParser(this.options));
        sequence.push((_ref1 = this.options.afterParser) != null ? _ref1 : _.identity);
        sequence.push(new Parser.MapIconDecorator(this.options));
        return _(sequence).reduce(this._parse, data);
      };

      Parser.prototype._parse = function(data, parser) {
        switch (false) {
    ...
    Parser.MapIconDecorator = (function() {
      function MapIconDecorator() {
        this._makeIcon = __bind(this._makeIcon, this);
        this.execute = __bind(this.execute, this);
      }
      ...
      MapIconDecorator.prototype.execute = function(entries) {
        ...
        for (_i = 0, _len = entries.length; _i < _len; _i++) {
          entry = entries[_i];
          if (entry.icon != null) {
            entry.icon = this._makeIcon(entry.icon);
          }
          if (entry.shadow != null) {
            entry.shadow = this._makeIcon(entry.shadow);
          }
        }
        return entries;
      };

      MapIconDecorator.prototype._makeIcon = function(data) {
        var key, result, val;
        result = _.clone(data);
        for (key in data) {
          val = data[key];
