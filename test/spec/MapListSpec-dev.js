  ...
  describe("MapList", function() {
    ...
    describe("Parser", function() {
      ...
      describe("::constructor", function() {
        ...
        return it("non arguments", function() {
          var parser;

          parser = new this.Parser;
          expect(parser.parser).toBe(parser.defaultParser);
          return expect(parser.afterParser).toBe(_.identity);
        });
      });
      describe("::execute", function() {
        beforeEach(function() {
          this.parser = new this.Parser;
          spyOn(this.parser, 'parse').andReturn(1);
          spyOn(this.parser, 'afterParser').andReturn(2);
          spyOn(this.parser, 'finallyParse').andReturn(3);
          return this.result = this.parser.execute(0);
        });
        afterEach(function() {
          delete this.parser;
          return delete this.result;
        });
        it("call parse", function() {
          expect(this.parser.parse).toHaveBeenCalled();
          return expect(this.parser.parse.calls[0].args[0]).toBe(0);
        });
        it("call afterParser", function() {
          expect(this.parser.afterParser).toHaveBeenCalled();
          return expect(this.parser.afterParser.calls[0].args[0]).toBe(1);
        });
        it("call finallyParse", function() {
          expect(this.parser.finallyParse).toHaveBeenCalled();
          return expect(this.parser.finallyParse.calls[0].args[0]).toBe(2);
        });
        return it("return value", function() {
          return expect(this.result).toBe(3);
        });
      });
      return describe("::defaultParser", function() {
        return it("data is XMLDoc", function() {});
      });
    });
